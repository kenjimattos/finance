...
- **Overview: navegação de mês agora permite avançar**: a seta "→" deixa de travar em `defaultMonth + 1` e passa a ser habilitada quando o mês seguinte tem lançamentos — seja em qualquer cartão (via `hasNextBillTransactions` agregado dos breakdowns) ou no fluxo de caixa (bank transactions ou manual entries). Projeções de fatura (`credit_card_bill`) são explicitamente excluídas do critério de cashflow porque existem para todo mês futuro em qualquer conta configurada e manteriam a seta sempre ativa; a presença de atividade de cartão já é capturada pelo caminho de `hasNextBillTransactions`. Uma query de cashflow do mês +1 é feita em paralelo só para isso.
