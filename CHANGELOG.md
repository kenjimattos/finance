...
- **Totais da divisão mostram o delta vs mês anterior.** Abaixo de cada um dos dois totais da divisão (**meu** e **dela**) aparece a variação frente à fatura anterior, no mesmo estilo do total da seção cartões — `▲ R$ 120,00 vs anterior` (accent quando subiu, verde quando caiu, oculto sem variação). O split-summary devolve `previousPartnerOwes` e `previousMyShare`; o Overview soma esses valores entre contas antes de calcular o delta.
