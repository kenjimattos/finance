...
- **Histórico do que o Pluggy entrega a cada sync.** O sync do cartão sobrescrevia o `raw_json` a cada atualização, então o jeito de cada emissor se comportar só aparecia por acidente. Foi assim que o Itaú mostrou que, ao passar uma compra de PENDING para POSTED, troca o ID e atrasa o horário em 3h: o PENDING tinha virado órfão e o `raw_json` dele parou de ser atualizado. Agora cada sync de conta vira uma linha em `sync_runs` (quantas transações vieram e o que o motor fez com elas), e cada payload diferente fica em `transaction_payloads`, com o primeiro e o último sync em que apareceu e a linha local em que foi aplicado. Payload repetido só avança o "visto por último", então a tabela cresce com as edições do Pluggy, não com a quantidade de syncs. Nada no app lê essas tabelas: são para análise. Para isso o sync passou a baixar todas as páginas de uma conta antes de gravar, e o motor recebe o conjunto completo de uma vez.
...
- **Sync do cartão: um ID novo só reaproveita uma linha se o ID antigo parou de vir.** Quando o Pluggy entrega uma transação com um ID que o app não conhece, mas com o mesmo conteúdo de uma linha já gravada, o sync precisa decidir se é a mesma compra reenviada (reconexão, ou o PicPay, que gera IDs novos a cada raspagem) ou uma segunda compra igual. Até agora a decisão era pelo horário, ao milissegundo, e isso erra nos dois sentidos: o Itaú muda o horário da compra em 3h quando ela sai de PENDING, e os registros antigos dele usam um horário fixo (`18:00:01`), que dá o mesmo instante para duas compras reais. O critério agora é se o ID da linha existente ainda vem no mesmo sync: se vem, são duas compras e as duas ficam; se parou de vir, é a mesma compra com ID novo. O horário completo passou a fazer parte do `identity_hash`, que antes só usava o dia, e uma migração recalcula os hashes já gravados.

- **Conciliação: o modelo passa a conhecer o layout da fatura de cada emissor.** Na fatura do PicPay o modelo seguia escolhendo como total os R$ 10.073,30 do "Total geral dos lançamentos" — o rótulo parece o total líquido, mas o PicPay deixa os estornos fora dele e dos subtotais e os soma à parte em "Créditos e estornos". Isso não dá para deduzir lendo o rótulo, então agora o prompt descreve o layout do emissor da conta (identificado pelo nome do cartão, já que o conector é "MeuPluggy" para todos): no PicPay, o líquido é "Despesas do mês" mais os "Créditos e estornos" negativos; no Itaú, é o "Total dos lançamentos atuais", e as linhas internacionais, o bloco de parcelas futuras e o repasse de IOF voltam a ser explicados. As descrições não trazem nenhum valor de exemplo — foi um exemplo com valor que fez o modelo inventar um "Repasse de IOF" antes. Quando só existem as partes impressas (bruto e créditos), o modelo pode calcular o líquido a partir delas e cita as duas no rótulo; somar as linhas continua proibido.

- **Conciliação de fatura refeita: a IA concilia, o código confere.** A conciliação quebrava a cada fatura nova porque o código tentava interpretar o que é trabalho de julgamento. Na fatura do PicPay de setembro, a IA leu as 100 linhas certas e até copiou o "Total da fatura" (R$ 9.972,58), mas o código comparava com o "Total geral dos lançamentos" (R$ 10.073,30, bruto, sem os estornos), disparava uma "correção" que empurrava o modelo a inverter estornos, e o prompt — cheio de exemplos do Itaú com valores concretos — fez o modelo inventar um "Repasse de IOF" de R$ 24,99 que não existe na fatura. Agora uma chamada só manda **o PDF em si** (o modelo vê colunas e seções por cartão, que o texto achatado perdia) junto com as linhas do app daquela fatura, e o modelo devolve tudo: cada linha com o trecho impresso, o total líquido escolhido com o rótulo copiado e o motivo da escolha, o pareamento com o app e o motivo de cada linha que só existe no app. O código não interpreta mais nada: confere que os pareamentos citam linhas que existem, uma vez cada, que toda linha do app foi analisada, e calcula ele mesmo a soma das linhas e as diferenças de valor — o que não passar vira aviso na tela, nunca é descartado em silêncio. Saíram o pareamento por heurísticas (mesma data → parcela → ±3 dias → similaridade de nome), o loop de reparo e as regras por emissor no prompt. A tela mostra o trecho impresso em cada linha e o motivo da IA, para que a leitura possa ser conferida. O modelo da conciliação pode ser diferente do da importação por screenshot (`OPENAI_RECONCILE_MODEL`), porque é um trabalho bem mais pesado.

- **Importação e conciliação de fatura passaram a usar a API da OpenAI.** A extração falava o formato da Anthropic (`@anthropic-ai/sdk`) e, em produção, passava pelo endpoint compatível do OpenRouter com um modelo gratuito — que ignorava o `tool_choice` forçado, respondia com o raciocínio em texto corrido até esgotar os tokens (304s numa fatura do PicPay) e nunca devolvia as transações; o navegador desistia da conexão antes do erro chegar. Agora o cliente é o SDK `openai` (Chat Completions), que serve tanto a OpenAI direta quanto o OpenRouter pelo formato nativo dele. A ferramenta usa `strict: true`, então na OpenAI o JSON sai restrito ao schema. As variáveis mudaram: `OPENAI_API_KEY`, `OPENAI_MODEL` (obrigatório quando há key — o boot falha sem ele, porque um modelo sem visão ou sem function calling é justamente o que quebrou) e `OPENAI_BASE_URL` (opcional; ausente = OpenAI). As `ANTHROPIC_*` deixaram de ser lidas: sem as novas, as duas features ficam desligadas (503). Também caíram de 5 para 2 as retentativas automáticas, porque cada uma refaz a leitura inteira, e o erro de "o modelo não chamou a ferramenta" agora informa o `finish_reason`.

- **O inbox de categorização foi desmontado em três peças.** O `TransactionInbox` tinha 865 linhas e acumulava coisas que não têm relação entre si: as doze mutações de transação, o formulário de lançamento manual e a renderização do inbox. Agora a invalidação de cache vive em `lib/useBillMutations.ts` e o formulário em `components/ManualTransactionForm.tsx`, sobrando 548 linhas de inbox de verdade. Sem mudança visível na tela — o formulário recebe os mesmos props e as mutações fazem as mesmas chamadas.

- **As chaves de cache do frontend viraram um factory tipado.** As ~70 `queryKey` do app eram literais escritos à mão em 16 arquivos — e, como o TanStack Query casa chave por prefixo, um literal divergente não dá erro de tipo nem de runtime: só deixa um painel desatualizado. Agora todas passam por `lib/queryKeys.ts` (`keys.transactions.ofItem(itemId)`, `keys.billBreakdown.all`…), onde cada domínio expõe o prefixo largo para invalidar e os construtores estreitos para consultar. Efeito colateral: o `/auth/me` do Dashboard usava `['authMe']` enquanto o resto do app usava `['auth']`, então a mesma resposta era buscada e guardada duas vezes; agora é uma entrada de cache só.

- **`lib/api.ts` deixou de ser três arquivos num só.** Tinha 801 linhas fazendo coisas distintas: o wrapper de `fetch` (36 linhas), os tipos de resposta do backend (403) e os métodos por endpoint (359). Os tipos eram metade do arquivo e, pior, eram o que a maioria dos consumidores queria — de 21 imports, a maior parte pedia um tipo, não o cliente. Agora são `lib/apiTypes.ts` e `lib/api.ts`, ~420 linhas cada, cada um com um trabalho. Não é um barrel: os 14 arquivos que precisam de tipo importam de `apiTypes` direto, porque re-exportar por `api.ts` manteria todo mundo apontando para lá e recriaria o mesmo balaio em outro arquivo. Os marcadores de seção do arquivo também foram consertados — o `// Endpoints` ficava 138 linhas acima do primeiro endpoint, e havia dois estilos de marcador competindo.

- **A documentação foi conferida contra o código, afirmação por afirmação.** Uma varredura por todo `.md`, pelo `.env.example` e pelas referências a endpoints e tabelas nos comentários, checando cada alegação no código em vez de ler por plausibilidade. O que estava errado: o `docs/api.md` não citava onze rotas vivas — as três de `import-fatura` (ou seja, a conciliação inteira), `PUT /transactions/:id/hidden`, `GET /bills`, `GET /cards`, `PUT /cards/:last4/group`, as duas de `/partner/cards` e `POST /admin/restore-db` — e documentava uma, `POST /manual-entries/:id/duplicate`, que não existe em lugar nenhum. O `docs/schema.md` não citava três tabelas (`transaction_hidden`, `balance_anchors` e a drenada `bill_payment_tags`), e ganhou a distinção entre `balance_snapshots` (o número que o conector reporta, não confiável) e `balance_anchors` (o que o usuário confirmou, preferido), que não estava escrita em lugar nenhum. O `APP_PASSWORD` citado em dois docs virou `USER_<NOME>_PASSWORD` há meses. E o `docs/pluggy.md` mandava consultar `packages/api/data/finance.sqlite`, arquivo que deixou de existir quando os bancos passaram a ser um por usuário. O README não mencionava a conciliação por PDF.

- **`.env.example`: porta errada no `CORS_ORIGIN` e uma var faltando.** Apontava para `http://localhost:5173` enquanto o dev server do web sempre rodou em `5174` — inofensivo em dev, porque o Vite faz proxy de `/api/*` e ninguém precisa de CORS, mas errado para quem fosse servir o frontend de outra origem. Agora vem comentado, com a nota de quando é necessário. O `USER_<NOME>_PARTNER` também não estava lá, apesar de ser lido pelo `config.ts` e ter linha no README.

### Removido

- **`recompute-hashes.ts`, um backfill de uso único já consumido.** Nasceu junto com o dedup por hash em abril e foi rodado uma vez em 02/05, quando o cap de tokens do slug subiu de 3 para 5 e invalidou todo `identity_hash` guardado — a entrada daquele fix no changelog é a única referência a ele em todo o repositório. Nunca entrou no `package.json` e nunca foi tocado depois. Estava quebrado desde 17/05, quando os bancos viraram um por usuário: ele abre `data/finance.sqlite`, caminho que não existe mais. E quebrado de um jeito silencioso — o `better-sqlite3` cria o arquivo no `new Database()`, então ele rodava até o fim contra um banco vazio e reportava sucesso. Apontado para um banco real, reescreveria a chave de identidade de todas as transações sincronizadas com o slug de hoje, o que só é o comportamento desejado no instante seguinte a uma mudança no algoritmo do slug. Se isso acontecer de novo: `git show e35150f:packages/api/src/scripts/recompute-hashes.ts`.

- **`api.getCardSettings` / `putCardSettings` e a interface `CardSettings`.** Clientes das rotas legadas `/card-settings/:itemId`, por item em vez de por conta, que o app abandonou em favor de `/account-settings/:accountId`. Zero call sites; a interface existia só para tipar as duas. As rotas continuam montadas — a tabela `card_settings` ainda guarda linhas pré-migração — mas agora estão inalcançáveis pelo app, e o `docs/api.md` registra que rota e tabela saem juntas quando isso deixar de valer.
...
- **Visão "acabando": as parcelas na última cobrança.** Terceira opção do mesmo toggle, mostrando só as parcelas em que o número é o último da série (2/2, 4/4…) — ou seja, o valor que sai da próxima fatura. O contador vai no próprio botão ("acabando (3)"), então dá para ver quantas séries terminam sem entrar na visão, e o cabeçalho passa a somar o que a visão está mostrando.
...
- **Conciliação de fatura: o total agora vem do PDF, não da soma das linhas lidas.** O "fatura (pdf)" da conciliação era reconstruído somando os lançamentos que a extração conseguiu ler — então qualquer linha lida errado virava um total errado, e a diferença contra o app aparecia como se fosse problema de categorização. Agora a extração copia os totais impressos no resumo do próprio emissor ("Total dos lançamentos atuais", "Total de encargos", "Total desta fatura") e é esse valor que vai para a comparação, com fallback para a soma das linhas em faturas que não imprimem resumo. Como os dois números passam a existir separadamente, quando eles não batem o app avisa ("as linhas lidas somam X — R$ Y a menos que o total impresso"), em vez de esconder a falha de leitura dentro do delta. Encargos (juros, multa, IOF de financiamento) aparecem como nota à parte: eles entram no total da fatura mas não são lançamentos, então não distorcem mais a comparação.

- **Conciliação: leitura da fatura do Itaú.** A extração por PDF tinha sido escrita em cima da fatura do PicPay e errava a do Itaú de várias formas — todas conferidas contra a fatura de agosto/2026, que fechou R$ 7.974,83 de lançamentos e era lida como R$ 7.367,86. Corrigido: **(1) lançamentos internacionais** — o Itaú imprime o valor em R$ na linha do estabelecimento e o valor na moeda de origem na linha de baixo (`10/07 RAILWAY… 27,05` / `5,00 USD 5,00`); a leitura pegava o segundo, subestimando a fatura (R$ 579,19 da Anthropic viravam R$ 107,06); **(2) repasse de IOF** — cobrança real do bloco internacional, era ignorada; **(3) parcelas** — o Itaú marca a parcela como `04/12` no fim do texto do estabelecimento, sem o prefixo `PARC` que o PicPay usa, então nenhuma parcelada era reconhecida como tal; **(4) datas** — `07/11` (parcela comprada em novembro) era relida como 11 de julho para caber na janela da fatura; **(5) linhas de continuação** ("eletronicos SAO PAULO" embaixo de cada lançamento) e o bloco "Compras parceladas - próximas faturas" agora são explicitamente ignorados.

- **Conciliação: passe de correção contra o total impresso.** Como a leitura é forçada a responder direto na ferramenta, o modelo não tem onde conferir a própria aritmética. Se as linhas extraídas não fecham com o total impresso na fatura, o app devolve a diferença para o modelo e pede uma releitura (até 2 rodadas), mantendo o resultado só quando ele chega mais perto — é assim que erros de sinal em estorno e valor em moeda estrangeira, que aparecem em uma linha ou outra a cada leitura, são pegos antes de virarem sugestão de inserção.
...
- **Blindagem do motor de sync contra payloads corrompidos da Pluggy.** A lógica de decisão do sync de cartão (atualizar / repost / ID reciclado / reconexão) foi extraída para um serviço testável (`syncCreditTransactions`, 20 testes usando os payloads reais do incidente PicPay de jul/2026 como fixtures) e ganhou quatro guardas: **(1) mutação-quimera suprimida** — quando a Pluggy reescreve um registro in-place mas `descriptionRaw` e valor ainda são do conteúdo antigo, nada é criado (era isso que cunhava lançamentos fantasma, ex. os R$ 209,58 de 20/07); **(2) salto de data implausível suprimido** — "repost" com mais de 45 dias de distância não move a linha (evita arrastar gasto antigo categorizado para a fatura aberta); **(3) anti-flip-flop** — o payload é comparado com todas as gerações de um provider ID, então reversões não cunham cópias infinitas; **(4) discriminador de timestamp** — duas compras reais iguais no mesmo dia (mesmo estabelecimento e valor, horários diferentes) deixam de ser engolidas pelo dedup de reconexão, que agora exige o instante idêntico ao milissegundo (o padrão da rotação diária de IDs do PicPay). Mutações suprimidas são auditadas em `transaction_sync_conflicts` com `kind='mutation-suppressed'` (uma vez por payload, sem repetição). **Linha cunhada por conflito nunca mais recebe categoria automática** — nasce no inbox para revisão humana; regras aprendidas não treinam com ela.

### Adicionado

- **Mostrar/esconder as transações ocultas no fluxo de caixa.** O ledger ganha o toggle "+ mostrar N ocultas" (ao lado do toggle de histórico): as linhas bancárias escondidas via "Esconder do fluxo de caixa" passam a poder ser exibidas esmaecidas, com selo "oculta" e ação "👁 Restaurar no fluxo de caixa" no menu ⋯ — antes, restaurar uma linha escondida por engano exigia mexer no banco na mão. Ocultas continuam fora de todos os saldos (abertura, corrente e fim de mês) — o toggle controla só a exibição, sem novo fetch: o `GET /cashflow` agora sempre devolve essas linhas com a flag `hidden` e o frontend filtra localmente.

- **Ocultar lançamentos da fatura.** Nova ação por linha no inbox: marcar um lançamento como oculto o exclui de todos os cálculos — total e breakdown da fatura, delta vs mês anterior, resumo de divisão (Dashboard, Overview e visão do parceiro) e conciliação por PDF — sem apagá-lo: ele permanece listado numa seção própria, e a ação é reversível. Diferente de remover a categoria (que o `applyLearnedRules` desfaz no próximo sync recategorizando a linha), ocultar é estável: regras aprendidas pulam linhas ocultas e não treinam com elas (`hit_count` intacto). Nasceu dos lançamentos fantasma do conector PicPay — registros pendentes antigos que a Pluggy reescreve in-place com outro estabelecimento/data, cunhando linhas que não existem na fatura real (ex.: os R$ 209,58 de 20/07/2026). Backend: tabela `transaction_hidden` (join table de trabalho do usuário, sobrevive a re-sync), `PUT /transactions/:id/hidden`, flag `hidden` no `GET /transactions`.
