...
6. **Provider ID not found, no orphan with that hash** → genuinely new transaction, insert (new local UUID). This includes a hash match against a row that **was** served: Pluggy returning both records side by side is proof of two purchases with identical content. Real cases: two metro taps of the same fare minutes apart on PicPay, and Itaú's older records, which all carry the same synthetic `18:00:01.000` time, so two same-day purchases of the same amount share an instant.

Why the orphan rule instead of a separate timestamp check: the time-of-day alone cannot tell "same purchase, new ID" from "another purchase". Itaú's synthetic `18:00:01` gives two real purchases the same instant, and a record re-served under a new ID keeps its instant exactly. Whether the old ID is still being served is what separates the two. (Itaú's PENDING→POSTED is a different animal: the POSTED carries a new ID **and** a time shifted by exactly -3h, so its hash never matches the PENDING's.)
