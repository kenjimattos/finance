...
All three require `ANTHROPIC_API_KEY`; without it they return 503 and the frontend hides the buttons. Mounted with their own 25mb JSON limit (base64 images). Blocked for demo users.

- `POST /transactions/import-fatura/extract { accountId, billOffset, images[] }` — up to 20 base64 screenshots (`png`/`jpeg`/`webp`/`gif`) of the issuer's statement. Claude vision returns structured rows (date, description, amount, card last4, installments, estorno→negative), each carrying the `billShift` that lands it in the viewed bill. Nothing is written — the rows go back for review. Prompt and `normalizeExtraction` live in [services/extractFatura.ts](../packages/api/src/services/extractFatura.ts).
...
- `POST /transactions/import-fatura/reconcile { accountId, billOffset, pdfText }` — diffs the issuer's closed-bill PDF against the viewed bill. Receives the **extracted text, never the file**: [lib/pdfText.ts](../packages/web/src/lib/pdfText.ts) runs `unpdf` in the browser so a password-protected statement is unlocked locally and its password (typically the holder's CPF or birth date) never leaves the machine. 422 when the text is under 100 chars (scanned PDF). Matching is pure and unit-tested — greedy amount-anchored tiers (same date → same installment X/Y → ±3 days → same merchant) plus a cent-drift pass, payments excluded on both sides — in [services/reconcileFatura.ts](../packages/api/src/services/reconcileFatura.ts). Returns missing-in-app (insertable via `commit`), amount mismatches, only-in-app, matched count, and both totals.
