...
- **Reconcile against the issuer's PDF** — upload the closed-bill statement and an LLM reads it and pairs it against what the app has: missing rows (insertable in one click), amount mismatches, rows only the app knows about — each with the printed line or the model's reason, so the reading can be audited. Code checks the answer (sums, every row accounted for) and flags what doesn't add up. Password-protected PDFs are unlocked in the browser: only the decrypted copy is uploaded, and the password never leaves the machine.
